...
def simple_median_integ(data, snr = 3, dphi = 10):
    ...
    # The following parts are for trial.
    ...
    for i in np.arange(1,35,1):
        curve[i] = np.median([curve[i-1], curve[i], curve[i+1]])
    curve[0] = np.median([curve[-1],curve[0],curve[1]])
    curve[-1] = np.median([curve[-2],curve[-1],curve[0]])
...
def mask_integ_gaussian_visual(data, snr = 3, smooth = 2, dphi = 10, save = True):
    ...
    for i in range(ew_row):
        for j in range(ew_col):
            if np.isnan(angle_integral[i][j]) == False:
                if phi[i][j]%(4*dphi)<=10:
                    angle_integral[i][j] = 5
                elif phi[i][j]%(4*dphi)<=20:
                    angle_integral[i][j] = 10
                elif phi[i][j]%(4*dphi)<=30:
                    angle_integral[i][j] = 15
                else:
                    angle_integral[i][j] = 20
            else:
...
def mask_integ_visual_trial(data, snr = 3, save = True):
    ...
    noisy_curve = copy.deepcopy(curve)
    ...
    plt.xlabel('Angular Direction [degrees]', fontproperties=font)
    plt.ylabel('EW/spaxel [Angstrom]', fontproperties=font)
    plt.plot(np.arange(0,360,10), noisy_curve, label = 'EW curve')
    plt.axvline(x = 90, linestyle = '--', color = 'r',label = r'90$\degree$')
    plt.axvline(x = 270, linestyle = '--', color = 'r',label = r'270$\degree$')
    plt.grid(color='grey', linestyle='-', linewidth=0.2)
    plt.legend(loc='upper right',prop=font)
...
def reduced_chi_square(data, dof = 6):
    result = least_sq_median_fit(data)
    curve = np.array(result[0])
    n_curve = curve / sum(curve)
    fit = result[1]
    n_fit = fit / sum(fit)
    chi_square = scipy.stats.chisquare(n_curve, f_exp =n_fit, ddof = dof)
    return data, chi_square[0], chi_square[1]
...
def gaussian_test(data):
    try:
        ...
        return data, scale_min, center_max, sig_max, (100 * res/36)
    except:
        pass
...
def chi_shell(data):
    try:
        result = reduced_chi_square(data)
